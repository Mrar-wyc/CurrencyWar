...
## 当前状态（v0.2 批次①②③已完成，勿重做）

- ✅ ①官方商店概率表+Lv10（`4130e36`）　✅ ②后台自动施技+后台强度（`f52bb81`）　✅ ③投资策略三选一 20 条银/金（`d9b98f2`）
- 现有内容：16 角色（含专属后台赋能）/ 10 羁绊 / 14 装备 / 20 策略 / 22 节点（每局 5 个策略点）
- ⬜ 待做：④角色池扩 24+ → ⑤星徽 → ⑥行动值倒计时 → ⑦词缀难度 → ⑧超频等
...
tests/        core(41项) + balance(40局) + soak(300局不变量) + diag(单局诊断) + bot.ts(自动对局)
...
5. **改玩法必跑平衡回归**：`[balance]` 胜率断言带 15%~98%（当前健康区 85~93%）；大幅给玩家加成的新系统必须配对冲的难度机制（先例：金策略自动 +4% 敌属性）。
...
- 战斗演出：我方右侧近景/敌人左侧远景、行动条左上竖排、战技点右下、技能名横幅；后台支援栏在画面左下
