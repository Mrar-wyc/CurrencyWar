...
## 当前状态（v0.2 路线①~⑦完成，⑧第一片完成）

- ✅ ①商店概率表+Lv10（`4130e36`）　✅ ②后台自动施技（`f52bb81`）　✅ ③投资策略三选一（`d9b98f2`）
- ✅ ④角色池 24 名+官方费用校准（`af304c4`）　✅ ⑤星徽 10 枚+装备池 41+财富宝钻（`b6f50d7`~`acb2361`）　✅ ⑥行动值倒计时（`83553cb`）　✅ ⑦词缀难度 10 条（`80f96ee`~`4da29d7`）　✅ ⑧上 投资环境 16 条+随行顾问×4（`fb259ac`）
- 现有内容：24 角色（含专属后台赋能）/ 10 羁绊（7 条扩档）/ 41 装备（简易 8+进阶 23+星徽 10）/ 20 策略 / 16 环境+4 随行顾问 / 22 节点 / 财富宝钻 / 行动值倒计时战斗制 / 敌人词缀 10 条
- ⬜ 待做：⑧下 超频模式/对手公司/顾问商店招募/概念股概率加权
...
tests/        core(88项) + balance(40局) + soak(300局不变量) + diag(单局诊断) + bot.ts(自动对局)
...
5. **改玩法必跑平衡回归**：`[balance]` 胜率断言带 15%~98%（⑧环境后实测 57%~73%）；大幅给玩家加成的新系统必须配对冲的难度机制（先例：金策略自动 +4% 敌属性）。
