...
- ✅ v0.2.2 深挖轮：三路审计后修 14 类问题（团队暴击双计、减防无下限、景元神君一次性、佩拉顾问空转、末节点首领失败被判通关、牌池只减不增、四费晋升满席溢出、存档脏数据 7 处、选择节点软锁、渲染白屏兜底、战斗中重绘泄漏渲染器、开局护盾不显示、静态层预渲染、类型收口）。测试 122 项
- ✅ v0.2.3 验收自动化：新增 jsdom 界面冒烟（`tests/ui-smoke.test.ts`）——启动真实应用、真实点击走完每个屏幕与浮层，断言"该屏在场且没触发渲染兜底"。渲染崩溃/白屏、DOM 契约、画布尺寸、流程连通性从此进 CI 门禁；**真实排版/字号/溢出/绘制内容仍靠人工逐屏截图**。测试 125 项
- ✅ v0.2.4 棱彩档：`GRADE_POINTS`（银0/金3/彩6）单一真源贯通敌人难度与情报面板；棱彩位面二起入池；6 条棱彩策略（完美开局/爆仓/藏一手/控制规模/买断制/淘金客）+ 官方明确属棱彩的两条改档。策略 20→26 条，测试 133 项
...
tests/        core(123项) + guards(5项跨文件护栏) + ui-smoke(3项 jsdom 界面冒烟) + balance(40局) + soak(300局不变量+终止分布) + diag(单局诊断) + bot.ts(自动对局)
