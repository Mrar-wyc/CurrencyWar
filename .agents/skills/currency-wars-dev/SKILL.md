...
- ✅ ④角色池 24 名+官方费用校准（`af304c4`）　✅ ⑤星徽 10 枚+装备池 41+财富宝钻（`b6f50d7`~`acb2361`）
- 现有内容：24 角色（含专属后台赋能）/ 10 羁绊（7 条扩档）/ 41 装备（简易 8+进阶 23+星徽 10）/ 20 策略 / 22 节点 / 财富宝钻
- ⬜ 待做：⑥行动值倒计时 → ⑦词缀难度 → ⑧超频等
...
5. **改玩法必跑平衡回归**：`[balance]` 胜率断言带 15%~98%（④⑤后实测 45%~68%，均值 ≈60%）；大幅给玩家加成的新系统必须配对冲的难度机制（先例：金策略自动 +4% 敌属性）。
