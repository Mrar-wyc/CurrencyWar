...
| 规则单测（115 项） | `tests/core.test.ts` | 商店/站位/经济/装备/星徽羁绊/宝钻/后台/策略/引擎/倒计时/词缀/节点/存档/阶段修复/结算与经济 | ~80ms |
...
| **UI 冒烟（jsdom）** | `tests/ui-smoke.test.ts` | 启动真实应用 + 真实点击走完全部屏幕与浮层；断言该屏在场且未触发渲染兜底、画布位图尺寸、一场战斗只结算一次、备战期已写续档 | ~3s |

全量命令：`npm run typecheck && npm test && npm run build`（`npm test` 默认跑全部 6 个文件 = 125 项）

> **UI 类缺陷的归属**：渲染崩溃/白屏、DOM 契约、画布尺寸、流程连通性 → 由 `ui-smoke` 在 CI 里兜底（改 UI 后先跑它）；
> **真实排版、字号、溢出、画布实际绘制内容 → 仍然只能人工逐屏截图验收**（`ui-smoke` 在 jsdom 里看不到这些）。
> 改 `ui-smoke` 前先读 `docs/开发进度.md` §十一：`render()` 的兜底会吞异常，断言必须同时检查"该屏在场"和"没有兜底 toast"。
...
**载档后也必须满足同一套不变量**：`migrateMatch` 负责把脏档钳回合法区间（数值标量、
商店、牌池、站位、uid 唯一、计数器 map、经验、布尔字段），`repairPhase` 负责让当前阶段
一定可推进。加新的 MatchState 字段时两处都要想一遍——漏一个就可能是白屏或软锁
（v0.2.2 深挖轮的 5 个 P0 全是这么来的）。
...
- v0.2.2 深挖轮（**含 4 处强度修正 + 末节点判负口径变更**）：balance 63/57/50/70/60%（5 次，均值 ≈60%），soak 300 局 47%~57%，带内无需校准，但**确实系统性下移了约 10pp**：①团队暴击此前被算两遍（白送一档暴击收益）②末节点首领失败原先也算通关。教训：**"修 bug" 也会改平衡**，而且改正后的数值更"难"——旧基线是带 bug 的难度，别为了恢复旧胜率再把 bug 的效果用别的方式加回去；强度类修正后必须重新基线，并留意 soak 下沿（40%）的余量。
- **采样噪声量级**：同一份代码连测 5 次即可见 50%~70%（≈20pp）跨度，故**单次 40 局结果不足以判断回调**，必须按下面的"每档重跑 3 次"执行；括号内断言带宽（15%~98%）正是为此设的宽。
- v0.2.3（仅新增验收自动化，未动数值）：soak 300 局连跑 5 次 = 49.3~57.0%（均值 ≈54.3%），距下沿 40% 余量充足；此前的单次 47% 属采样噪声。**soak 的 band 断言现在带说明性失败信息**（直接告诉你这是平衡漂移、去按回退顺序处理，而不是状态腐坏）。
